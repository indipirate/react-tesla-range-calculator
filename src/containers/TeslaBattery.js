...
class TeslaBattery extends React.Component {
  constructor(props) {
    ...
    this.state = {
      carstats: [],
      config: { speed: 55, temperature: 20, climate: true, wheels: 19 },
    };
  ...
  updateCounterState(title, newValue) {
    const config = { ...this.state.config }; // update config state with new value
    title === "Speed"
      ? (config["speed"] = newValue)
      : (config["temperature"] = newValue); // update our state
